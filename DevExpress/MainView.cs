...
namespace Furmanov.UI
{
	...
	{
		#region Fields
		private SalaryPayVisual _payCash;
		...
		public MainView()
		{
			...
			treeSalary.DataSource = new System.ComponentModel.BindingList<Furmanov.Dal.Dto.SalaryPayVisual>() /* TODO: Assign the real data here.*/;
		}
		...
		{
			VedomostClick?.Invoke(this, _payCash.ObjectId);
		}
		...
		{
			if (_payCash.Type == ObjType.Salary)
			{
		...
		{
			if (_payCash.Type != ObjType.Salary) return;
			var resName = _payCash.Name;
			if (_payCash.FactDays > 0)
			{
		...
		{
			if (treeSalary.Tag?.Equals(TreeListStateSaver.State.Updating) ?? false) return;
			if (treeSalary.GetRow(treeSalary.FocusedNode?.Id ?? -1) is SalaryPayVisual vm)
			{
				_payCash = vm;
